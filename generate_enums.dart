...
class SynthizerEnumMember {
  /// Create an instance.
  SynthizerEnumMember({required this.dartName, required this.value});
...
Future<void> main() async {
    ...
    if (currentEnum == null) {
      ...
      if (match != null) {
        ...
        currentEnum.members.add(SynthizerEnumMember(
            dartName: getDartName(dartMemberName), value: memberValue));
      }
    }
  }
  ...
  for (final e in enums) {
    ...
    for (final m in e.members) {
      buffer
        ..writeln('/// ${e.synthizerName} = ${m.value}')
        ..writeln('${m.dartName},');
    }
    buffer
      ..writeln('}')
      ...
      ..writeln('switch (this) {');
    for (final m in e.members) {
      buffer.writeln('case ${e.dartName}.${m.dartName}: return ${m.value};');
    }
    buffer
      ..writeln('}}}')
      ..writeln('/// An extension for converting integers to Dart values.')
      ..writeln('extension IntTo${e.dartName} on int {')
      ..writeln('/// Return a Dart value.')
      ..writeln('${e.dartName} to${e.dartName}() {')
      ..writeln('switch(this) {');
    for (final m in e.members) {
      buffer.writeln('case ${m.value}: return ${e.dartName}.${m.dartName};');
    }
    buffer
      ..writeln('default:')
      ..writeln(
          "throw SynthizerError('Unrecognised ${e.dartName} value.', this);")
      ..writeln('}}}');
  }
